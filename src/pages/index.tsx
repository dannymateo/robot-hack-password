...
import { Button, Card, CardBody, CardHeader, Chip, Input, Progress } from "@nextui-org/react";
import { 
  Lock, 
  Unlock, 
  AlertTriangle, 
...
export default function Home() {
  ...
  const startHacking = async () => {
    ...
    if (stats.timeEstimation.includes("años") && !isRealAttempt) {
      showAlert("Se recomienda usar el modo simulación para contraseñas largas");
    }
  ...
  const handleModeSelection = async (isReal: boolean) => {
    ...
    setStats(prev => ({
      ...
      currentStrategy: isReal ? "Iniciando ataque de fuerza bruta real" : "Iniciando simulación de fuerza bruta"
    }));
  ...
  return (
    <>
      <ToastContainer />
      <div className={`${geistSans.variable} min-h-screen bg-[#1a1b2e] text-white`}>
        {/* Grid sutil */}
            ...
            <CardBody className="space-y-4">
              <div className="flex items-start gap-3 border-l-4 border-[#3b82f6] pl-4 bg-[#3b82f6]/5 rounded-r-lg p-4">
                ...
                <div className="space-y-2">
                  <p className="text-[#3b82f6] font-semibold">ALERTA DE SEGURIDAD</p>
                  <p className="text-sm text-[#3b82f6]/80">Acceso no autorizado detectado en el servidor principal.</p>
                  <p className="text-sm text-[#3b82f6]/80">Se requiere recuperación inmediata del sistema.</p>
                </div>
              </div>
            </CardBody>
          </Card>
            ...
            <CardBody className="space-y-6">
              <div className="relative">
                <Input
                  ...
                  classNames={{
                    ...
                    label: "text-[#3b82f6]",
                    inputWrapper: "bg-[#1a1b2e]/50 border-[#3b82f6]/50 backdrop-blur-xl"
                  }}
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  size="lg"
                  startContent={isHacking ? <RefreshCw className="w-6 h-6 animate-spin" /> : <Zap className="w-6 h-6" />}
                >
                  <span className="relative z-10 font-bold text-white">
                    {isHacking ? "DESCIFRANDO..." : "INICIAR PROCESO DE RECUPERACIÓN"}
                  </span>
                </Button>
                ...
                {(isHacking || foundPassword) && (
                  <Button
                    ...
                    size="lg"
                    startContent={<RefreshCw className="text-white/90 w-6 h-6" />}
                  >
                  </Button>
                )}
              </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-white/90 ${isRealAttempt ? "text-red-400/90" : ""}`}>
                        {isRealAttempt ? "PROGRESO DE ATAQUE REAL" : "PROGRESO DE SIMULACIÓN"}
                      </span>
                    </div>
                    <Chip
                      variant="flat"
                ...
                <div className="space-y-4">
                  ...
                  <div className="font-mono text-xl bg-white/10 p-6 rounded-lg border border-white/30 backdrop-blur-xl">
                    <div className="flex justify-between items-center mb-4">
                      <span className="text-sm text-white/90">Último intento:</span>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="p-4 rounded-lg bg-white/10 border border-white/30 backdrop-blur-xl">
                      ...
                      <p className="font-mono text-white font-bold">{currentLength} caracteres</p>
                    </div>
                    <div className="p-4 rounded-lg bg-white/10 border border-white/30 backdrop-blur-xl">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="flex items-center gap-2">
                      ...
                      <p className="font-mono text-white font-bold">{attemptCount.toLocaleString()}</p>
                    </div>
                  </div>
                </div>
              </CardBody>
              ...
              <CardBody className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="p-4 rounded-lg bg-[#1a1b2e]/70 border border-[#3b82f6]/30">
                    <h3 className="text-lg font-bold text-[#3b82f6] mb-3">Análisis de Seguridad</h3>
                    
                    <div className="space-y-4">
                      <div className="bg-[#1a1b2e]/50 p-4 rounded-lg">
                        <h4 className="text-white/90 font-semibold mb-2">Escenario:</h4>
                        <p className="text-sm text-white/80">
                          Un grupo de hackers ha comprometido el servidor de una empresa debido a malas prácticas de seguridad.
                          La contraseña del administrador debe tener entre 4 y 8 caracteres, usando solo letras mayúsculas y minúsculas.
                        </p>
                      </div>

                      <div className="bg-[#1a1b2e]/50 p-4 rounded-lg">
                        <h4 className="text-white/90 font-semibold mb-2">Análisis de Posibilidades:</h4>
                        <div className="space-y-2">
                          <div className="flex justify-between items-center">
                            <span className="text-white/80">Total de combinaciones posibles:</span>
                      ...
                      <div className="bg-[#1a1b2e]/50 p-4 rounded-lg">
                        <h4 className="text-white/90 font-semibold mb-2">Principios de Conteo:</h4>
                        <p className="text-sm text-white/80">
                          Utilizamos el principio multiplicativo: para cada posición tenemos 52 opciones 
                          (26 minúsculas + 26 mayúsculas). El total de posibilidades para cada longitud 
                          es 52^n, donde n es la longitud de la contraseña.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#1a1b2e]/50 p-4 rounded-lg mt-4">
                    <h4 className="text-white/90 font-semibold mb-2">Distribución por Longitud:</h4>
                    {Object.entries(stats.possibleCombinationsInfo).map(([length, combinations]) => {
                      const percentage = (combinations / stats.totalPossibleAll * 100);
                      return (
                        <div key={length} className="flex justify-between items-center mb-2">
                          <span className="text-white/80">
                            {length} caracteres ({combinations.toLocaleString()} combinaciones):
                          </span>
                    ...
                    <div className="mt-4 p-4 bg-[#3b82f6]/10 rounded-lg border border-[#3b82f6]/30">
                      <h4 className="text-white/90 font-semibold mb-2">Análisis de la Contraseña Actual:</h4>
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          <span className="text-white/80">
                            Combinaciones totales para {currentLength} caracteres:
                          </span>
                          <span className="font-mono text-[#3b82f6]">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-mono text-[#3b82f6]">
                            {stats.fourLetterCombinations.toLocaleString()} ({formatPercentage(stats.fourLetterPercentage)}% del total)
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              <CardBody className="space-y-4">
                <div className="bg-[#3b82f6]/10 p-4 rounded-lg border border-[#3b82f6]/30">
                  ...
                  <p className="text-sm text-[#3b82f6] mt-2">
                    Este modo realizará un ataque de fuerza bruta real probando cada combinación posible:
                  </p>
              ...
              <div className="text-center space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3 bg-[#1a1b2e]/70 p-3 rounded-lg border border-[#3b82f6]/20">
                      <User className="w-5 h-5 text-[#3b82f6]" />
                      <p className="text-white/90">José Ricardo Quirós García</p>
                    </div>
                    <div className="flex items-center gap-3 bg-[#1a1b2e]/70 p-3 rounded-lg border border-[#3b82f6]/20">
                      <User className="w-5 h-5 text-[#3b82f6]" />
                      <p className="text-white/90">Diego Armando Ramírez Duque</p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div className="flex items-center gap-3 bg-[#1a1b2e]/70 p-3 rounded-lg border border-[#3b82f6]/20">
                      <User className="w-5 h-5 text-[#3b82f6]" />
                      <p className="text-white/90">Jean Carlos Gonzalez Goyeneche</p>
                    </div>
                    <div className="flex items-center gap-3 bg-[#1a1b2e]/70 p-3 rounded-lg border border-[#3b82f6]/20">
                      <User className="w-5 h-5 text-[#3b82f6]" />
                      <p className="text-white/90">Danny Mateo Hernández Sánchez</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-[#3b82f6]/20">
                  <div className="flex flex-col items-center justify-center gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-white/90 font-semibold">Universidad ITM</span>
                    </div>
